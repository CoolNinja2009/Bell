...
constexpr uint32_t HTTP_TIMEOUT_MS   = 5000;          // standard HTTP timeout (used for heartbeat, logs, etc)
...
//
// IMPORTANT: This code assumes the RTC is configured in 24-hour mode.
// If your RTC is in 12-hour mode, times will read/write incorrectly.
// Verify or set 24-hour mode on your specific RTC module before deployment.
...
constexpr uint32_t TASK_WDT_TIMEOUT_S      = 30;     // hardware watchdog: reset if no loop() for 30s
...
static bool fetch_schedule() {
    ...
    if (err == DeserializationError::NoMemory) {
        DBGLN(F("[JSON] buffer overflow — config too large! Increase StaticJsonDocument size."));
        return false;
    }
    if (err) {
...
void setup() {
    ...
    // --- Hardware watchdog — 30s timeout, reset ESP if loop() stalls ---
    esp_task_wdt_init(TASK_WDT_TIMEOUT_S, true);
    esp_task_wdt_add(nullptr);
    Serial.printf("WDT: enabled (%us timeout)\n", TASK_WDT_TIMEOUT_S);
...
void loop() {
    ...
    // ── 10b2.  BLE retry (if no beacon seen yet) — discover every 30s ──
    ...
    // ── 10i.  Periodic schedule refresh (DST / drift) ──────────────────
    if (s_schedules_seeded
    ...
    // ── 10j.  Tick both channels ───────────────────────────────────────
    tick_channel(g_ch1, now);
    tick_channel(g_ch2, now);

    // ── 10k.  Feed the hardware watchdog (required for 24/7 operation) ──
    esp_task_wdt_reset();
}