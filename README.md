...
The Profile Editor (`/profile-editor`) isn't shown as its own box above — it sits alongside the Profile Manager and writes through the same `profiles.json`, but goes through a validate → repair → restore pipeline first. See `server-node/README.md` for the full flow.
...
| `server-node/firmware_state.json` | Firmware source selection, auto-update flag, force-bypass state |
| `server-node/profiles.json.bak` | Automatic pre-write snapshot, used by Profile Editor's Restore |
...
### Profile Editor & Repair (`/profile-editor`)

A raw JSON editor for `profiles.json`, for when something needs fixing below the level the Profile Manager UI edits at:

- **Live validation** — checks syntax → JSON Schema → business rules as you type, before you save
- **Attempt Automatic Repair** — best-effort rebuild of a broken file; never invents a bell time that wasn't already there, and previews the diff before writing anything
- **Backup / Restore** — one-click restore from the automatic `.bak` snapshot taken before the last risky write
- **System health check** — flags if `settings.json`, `calendar.json`, `api_keys.json`, or `firmware_state.json` are unreadable, without needing to open each one

Every write to `profiles.json` — from this editor, the Profile Manager, import, or the API — goes through the same validation pipeline, so a bad edit can't silently corrupt the schedule the ESP32 is running from.
...
| `GET` | `/profile-editor` | Raw JSON profile editor page |
| `GET` | `/api/system/health` | Broken/healthy status for settings, calendar, API keys, firmware state files |
| `GET`/`POST` | `/api/profile/validate` | Check on-disk (GET) or candidate text (POST) against the validation pipeline |
| `GET` | `/api/profile/raw` | Current on-disk `profiles.json` text, even if broken |
| `POST` | `/api/profile/save` | Validate then atomically save `profiles.json` |
| `POST` | `/api/profile/repair` | Preview a best-effort automatic rebuild (no write) |
| `POST` | `/api/profile/repair/apply` | Write a previewed rebuild (requires `confirm: true`) |
| `GET` | `/api/profile/backup` | Whether a `.bak` snapshot exists |
...
- **Self-healing profiles.json** — validation pipeline (syntax → schema → business rules) blocks bad writes before they land; automatic repair preview and one-click backup restore if it's already broken
